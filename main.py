...
class MainUi(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.OMDocumentBttn.clicked.connect(self.open_pdf)
        self.ui.OMAboutBttn.clicked.connect(self.open_pdf)
        self.ui.OMLogOutBttn.clicked.connect(self.run_second_script)
        self.ui.OMAccountBttn.clicked.connect(self.display_user_info)
        self.ui.LogGenerateBttn.clicked.connect(self.run_third_script)
    ...
    def open_pdf(self):
        ...
        pdf_file_path = "D:\\NetAdminPro\\FINAL PROJECT ANJANI & AMAN.pdf"
        url = QUrl.fromLocalFile(pdf_file_path)
        QDesktopServices.openUrl(url)
        
    def open_pdf2(self):
        # Replace 'path/to/your/pdf/file.pdf' with the actual path of your PDF file
            pdf_file_path2 = 'D:\\NetAdminPro\\FINAL PROJECT ANJANI & AMAN.pdf'
            url = QUrl.fromLocalFile(pdf_file_path2)
            QDesktopServices.openUrl(url)
            
    def run_second_script(self):
            try:
                # Replace 'second_script.py' with the actual name of your second script
                subprocess.run(['python', 'D:\\NetAdminPro\\Login.py'], check=True)
            except subprocess.CalledProcessError:
                print("Error running second script")
